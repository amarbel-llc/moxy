...
function restack_autosquashes_a_fixup_against_an_ancestor_branch { # @test
  ...
  [ "$new_a" != "$old_a" ] ||
    {
      echo "expected $STACK_BRANCH_A SHA to change after restack; got $old_a → $new_a (--update-refs may be silently dropped)" >&2
      false
    }
  [ "$new_b" != "$old_b" ] ||
    {
      echo "expected $STACK_BRANCH_B SHA to change after restack; got $old_b → $new_b (--update-refs may be silently dropped)" >&2
      false
    }
  [ "$new_c" != "$old_c" ] ||
    {
      echo "expected $STACK_BRANCH_C SHA to change after restack; got $old_c → $new_c (--update-refs may be silently dropped)" >&2
      false
    }
}
...
function restack_failure_can_be_recovered_via_grit_rebase_abort { # @test
  ...
  [ -d "$STACK_WORK/.git/rebase-merge" ] || [ -d "$STACK_WORK/.git/rebase-apply" ] ||
    {
      echo "expected a rebase to be in progress under $STACK_WORK/.git/" >&2
      false
    }
  ...
  [ ! -d "$STACK_WORK/.git/rebase-merge" ] ||
    {
      echo "rebase-merge directory still exists after grit.rebase --abort" >&2
      false
    }
  [ ! -d "$STACK_WORK/.git/rebase-apply" ] ||
    {
      echo "rebase-apply directory still exists after grit.rebase --abort" >&2
      false
    }
  cd "$STACK_WORK"
