...
try {
  ...
  const result = await $`go doc -all -src ${arg}`.quiet();
...
try {
  ...
  const arg = docArg(target, symbol);
  const result = await $`go doc -all -src ${arg}`;
  process.stdout.write(result.stdout);
} catch (err) {
  process.stderr.write(
    `src: cannot find package "${pkg}"${symbol ? `.${symbol}` : ""}: ${err instanceof Error ? err.message : err}\n`,
  );
  process.exit(1);
}
