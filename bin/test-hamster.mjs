...
function assertContains(haystack, needle, label) {
  if (!haystack.includes(needle)) {
    throw new Error(
      `${label}: expected to contain ${JSON.stringify(needle)}; got ${haystack.slice(0, 200)}`,
    );
  }
}
...
function assertNotContains(haystack, needle, label) {
  if (haystack.includes(needle)) {
    throw new Error(
      `${label}: expected to NOT contain ${JSON.stringify(needle)}`,
    );
  }
}
...
await $`just -f ${path.join(REPO_ROOT, "justfile")} build-moxins`.pipe(
  process.stderr,
);
console.log("");
...
await test("parent package appends ## Sub-packages section (encoding)", async () => {
  ...
  assertContains(
    out,
    "## Sub-packages",
    "encoding should list sub-packages section",
  );
  assertContains(
    out,
    "encoding/json",
    "sub-packages should include encoding/json",
  );
});

await test("symbol query suppresses sub-packages (encoding.BinaryMarshaler)", async () => {
  const out = (
    await $({ cwd: REPO_ROOT })`${bin("doc")} encoding BinaryMarshaler`
  ).stdout;
  assertContains(out, "BinaryMarshaler", "symbol body present");
  assertNotContains(
    out,
    "## Sub-packages",
    "symbol query must skip sub-package listing",
  );
});
...
await test("external module sub-package resolves via GOMODCACHE", async () => {
  ...
  assertContains(
    out,
    `import "${PKG}"`,
    "should render the resolved sub-package",
  );
});
...
await test("symbol slices to that symbol section (#188)", async () => {
  ...
  assertContains(
    out,
    '<a name="Println">',
    "should include the Println anchor",
  );
  assertContains(out, "func Println", "should include Println signature");
  assertNotContains(
    out,
    '<a name="Printf">',
    "should not bleed into the next symbol",
  );
...
await test("missing symbol errors with available-anchor hint", async () => {
  ...
  if (exitCode === 0)
    throw new Error("expected nonzero exit for missing symbol");
  if (!stderr.includes("not found")) {
    throw new Error(
      `expected 'not found' in stderr; got: ${stderr.slice(0, 200)}`,
    );
  }
  if (!stderr.includes("Available anchors")) {
    throw new Error(
      `expected anchor list in stderr; got: ${stderr.slice(0, 200)}`,
    );
  }
});
...
await test("doc-outline lists exported anchors with kind context", async () => {
  ...
  assertContains(
    out,
    "func Println",
    "should render the kind + name from gomarkdoc",
  );
...
await test("doc-outline includes unexported with unexported=true", async () => {
  ...
  const out = (await $({ cwd: REPO_ROOT })`${bin("doc-outline")} fmt "" true`)
    .stdout;
  assertContains(
    out,
    "ldigits",
    "should include unexported anchor when unexported=true",
  );
});
...
await test("doc-outline labels vars/consts via section context", async () => {
  ...
  const out = (await $({ cwd: REPO_ROOT })`${bin("doc-outline")} errors`)
    .stdout;
  assertContains(
    out,
    "ErrUnsupported  # var ErrUnsupported",
    'errors.ErrUnsupported should be labeled "var"',
  );
  ...
  assertContains(
    ioOut,
    "# const SeekStart",
    'SeekStart should be labeled "const" via Constants section context',
  );
  assertContains(ioOut, "EOF", "should include the EOF variable");
  assertContains(
    ioOut,
    "# var EOF",
    'EOF should be labeled "var" via Variables section context',
  );
});
...
await test("doc-outline labels inline-anchored sentinels via CodeBlock fallback (#190)", async () => {
  ...
  assertContains(
    ioOut,
    "# var Discard",
    'io.Discard should be labeled "var" via CodeBlock fallback',
  );
  const timeOut = (await $({ cwd: REPO_ROOT })`${bin("doc-outline")} time`)
    .stdout;
  assertContains(
    timeOut,
    "# const Sunday",
    'time.Sunday should be labeled "const" via CodeBlock fallback',
  );
  const osOut = (await $({ cwd: REPO_ROOT })`${bin("doc-outline")} os`).stdout;
  assertContains(
    osOut,
    "# var Interrupt",
    'os.Interrupt should be labeled "var" via CodeBlock fallback',
  );
});
...
await test("doc-outline hides Type.unexportedMethod by default", async () => {
  ...
  assertNotContains(
    ...
    "unexported method on exported type should be hidden by default",
  );
  const withUnexp = (
    await $({ cwd: REPO_ROOT })`${bin("doc-outline")} ${PKG} "" true`
  ).stdout;
  assertContains(
    withUnexp,
    "Handler.handleInitialize",
    "unexported method should appear with unexported=true",
  );
});
...
await test("tags surfaces tag-gated symbols (#185)", async () => {
  ...
  try {
    ...
    assertContains(
      out,
      "Regular",
      "gomarkdoc should render the default-tag type",
    );
    assertContains(
      out,
      "Tagged",
      "gomarkdoc with --tags=special should render the tagged type",
    );
  } finally {
...
await test("stdlib symbol (fmt.Println)", async () => {
  ...
  assertContains(
    out,
    "func Println",
    "src should print the function definition",
  );
});
...
await test("cwd flag points go-vet at a subdirectory module (#174)", async () => {
  ...
  try {
    ...
    fs.mkdirSync(sub);
    fs.writeFileSync(
      path.join(sub, "go.mod"),
      "module hamstertest\n\ngo 1.22\n",
    );
    fs.writeFileSync(
      path.join(sub, "main.go"),
      "package main\n\nfunc main() {}\n",
    );
...
await test("tags flag includes test files behind //go:build (#185)", async () => {
  ...
  try {
    ...
    try {
      await $({
        cwd: tmp,
      })`${bin("go-test")} ./... "" false "" false false "" "" false false "" special`.quiet();
    } catch (e) {
      if ((e.stdout || "").includes("HAMSTER_TAGGED_RAN")) sawMarker = true;
    }
    if (!sawMarker) {
      throw new Error(
        "expected tagged test to compile and run with tags=special",
      );
    }
  } finally {
    fs.rmSync(tmp, { recursive: true, force: true });
...
await test("./cmd/moxy → /tmp/moxy-test-build", async () => {
  ...
  try {
    await $({
      cwd: REPO_ROOT,
    })`${bin("go-build")} ./cmd/moxy ${outPath} "" "" false`;
    if (!fs.existsSync(outPath))
      throw new Error(`expected ${outPath} to exist`);
...
await test("default output discards binary; CWD stays clean (#198)", async () => {
  ...
  try {
    ...
    const leftover = fs
      .readdirSync(tmp)
      .filter((f) => f !== "go.mod" && f !== "main.go");
    if (leftover.length > 0) {
      throw new Error(
        `go-build with no output arg left stray files: ${leftover.join(", ")}`,
      );
    }
  } finally {
    fs.rmSync(tmp, { recursive: true, force: true });
