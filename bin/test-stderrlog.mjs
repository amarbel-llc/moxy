...
try {
  await $`just -f ${path.join(REPO_ROOT, "justfile")} build-go`.pipe(
    process.stderr,
  );
} catch (e) {
...
function orphanPath(sessionID) {
  return path.join(
    STDERR_ROOT,
    "completed",
    sanitizeSessionID(sessionID) + ".orphan.log",
  );
}
...
console.log("\n=== Phase 1: clean lifecycle (init → stdin close → rotate) ===");
{
  ...
  if (!appeared) {
    ...
    console.log(
      `  (diag) moxy stdout (first 500 chars):\n    ${stdout.slice(0, 500)}`,
    );
    console.log(
      `  (diag) child.exitCode=${child.exitCode} child.signalCode=${child.signalCode}`,
    );
  }
  assert(
    await fs.pathExists(activePath(SESSION)),
    "active/<id>.log exists during run",
  );

  child.stdin.end();
  await new Promise((resolve) => child.on("exit", resolve));

  const bannerContent = await fs
    .readFile(completedPath(SESSION), "utf8")
    .catch(() => "");
  assert(
    !(await fs.pathExists(activePath(SESSION))),
  ...
  assert(
    bannerContent.includes("--- moxy started"),
    "banner line present in rotated file",
  );
  assert(
    bannerContent.includes("--- moxy stopping"),
    "shutdown line present in rotated file",
  );
}
...
);
{
  ...
  if (stillAlive) {
    console.log(
      `  (note: pid ${killedPid} still visible in /proc — skipping orphan sweep check)`,
    );
  } else {
...
console.log("\n=== Phase 4: SessionEnd hook rotates active → completed ===");
{
  ...
  assert(
    await fs.pathExists(activePath(SESSION)),
    "active/<id>.log exists before hook",
  );
