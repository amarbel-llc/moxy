...
const POC_ENTRYPOINT = path.join(REPO_ROOT, 'zz-pocs', 'list-changed', 'serve.ts')
...
const addr = await new Promise((resolve, reject) => {
  ...
  const onExit = (code) => settle(reject, new Error(`POC exited before handshake (code ${code}); see ${LOG_FILE}`))
...
if (!ready) {
  console.error(`POC /healthz never became 200 at ${healthURL}; see ${LOG_FILE}`)
  poc.kill()
  process.exit(1)
}
...
console.log('     - If Claude says it can\u2019t find the tool → registry stale.')
console.log('')
